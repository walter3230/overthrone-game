...
class RingPage extends StatefulWidget {
  const RingPage({super.key, required this.heroName});
...
class _RingPageState extends State<RingPage> {
  // Persist keys
  static const _eqKeyPrefix = 'eq_ring_'; // hero slot key (JSON of equipped)
  static const _invKey = 'inv_ring'; // global inventory list
  ...
  EqItem? _equipped; // equipped on this hero (we DO NOT keep in inventory)
  ...
  Future<void> _loadAll() async {
    ...
    if (rawEq != null && rawEq.trim().isNotEmpty) {
      try {
        ...
        _removeOneFromInventory(_equipped!); // drop one copy from inventory
      } catch (_) {
        _equipped = null;
      }
    }
  ...
  Future<void> _equip(EqItem it) async {
    _removeOneFromInventory(it); // drop one copy if exists

    if (_equipped != null) {
      _items.insert(0, _equipped!); // return old to inventory
    }
    ...
    ScaffoldMessenger.of(
      context,
    ).showSnackBar(const SnackBar(content: Text('Ring equipped.')));
    setState(() {});
  }
        ...
        child: ListView(
          ...
          children: [
            ListTile(
              ...
              subtitle: const Text(
                r'$100 • 10K Crystal • Mythic Ring • Stone Box (100)',
              ),
              onTap: () async {
            ...
            ListTile(
              ...
              subtitle: const Text(
                r'$50 • 5K Crystal • 50 Ring Shards • Stone Box (50)',
              ),
              onTap: () async {
                Navigator.pop(context);
  ...
  Widget _empty(ColorScheme cs) => Container(
    ...
    child: const Row(
      children: [
        ...
        SizedBox(width: 12),
        Expanded(child: Text('No ring equipped.')),
      ],
    ),
  );
  ...
  Widget _rarityIcon(EqRarity r) => CircleAvatar(
    ...
    child: Icon(Icons.ring_volume, color: r.color(context)),
  );
  ...
  Widget _filtersRow() {
    Widget chip(EqRarity r) => FilterChip(
      ...
      avatar: Icon(Icons.ring_volume, size: 18, color: r.color(context)),
    );
    return Wrap(
      spacing: 10,
      runSpacing: 10,
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                Icon(Icons.ring_volume, color: it.rarity.color(context)),
            ...
            child: Column(
              ...
              children: [
                ListTile(
                  ...
                  title: Text('Ring · ${it.rarity.label}'),
                ...
                ListTile(
                  ...
                  subtitle: const Text('Costs: 100× VoidStone • +2% True DMG.'),
  ...
  List<String> _bonusLines(EqItem it) {
    ...
    final atkPct = (0.04 * t) + (0.01 * it.level);
    final cr = (0.01 * t) + (0.001 * it.level);
    final cd = (0.12 * t) + (0.02 * it.level);
    final lines = <String>[
      'ATK%: +${pct(atkPct)}',
      'Crit Rate: +${pct(cr)}',
      'Crit DMG: +${pct(cd)}',
