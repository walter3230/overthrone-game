...
class HelmetPage extends StatefulWidget {
  const HelmetPage({super.key, required this.heroName});
...
class _HelmetPageState extends State<HelmetPage> {
  // Persist keys
  static const _eqKeyPrefix = 'eq_helmet_'; // hero slot key (JSON of equipped)
  static const _invKey = 'inv_helmet'; // global inventory list

  // Inventory (seed örnekleri)
  List<EqItem> _items = [
  ...
  EqItem? _equipped; // bu kahramanda takılı olan (envanterde TUTMAYIZ)
  ...
  /// Envanterde eşleşen tek bir kopyayı bulup çıkar
  /// (Yoksa false döner; varsa çıkartıp true döner)
  ...
  Future<void> _loadAll() async {
    ...
    // Slot: JSON saklıyoruz (models’a dokunmadan)
    ...
    if (rawEq != null && rawEq.trim().isNotEmpty) {
      try {
        ...
        // Envanterde varsa BİR kopyasını düş (tek kopya garantisi)
  ...
  // ------------------------------ actions ------------------------------
  /// Equip: envanterden 1 kopyayı düş, eski ekipmanı envantere geri koy
  Future<void> _equip(EqItem it) async {
    // önce envanterden bu item’ın bir kopyasını düş
    final removed = _removeOneFromInventory(it);
    if (!removed) {
      // güvenlik: envanterde yoksa da force equip (duplikasyon olmasın)
    }

    // eski takılıyı envantere geri koy
    if (_equipped != null) {
    ...
    ScaffoldMessenger.of(
      context,
    ).showSnackBar(const SnackBar(content: Text('Helmet equipped.')));
    setState(() {});
  }

  /// Unequip: takılı olanı envantere iade et
  ...
  /// Forge: SADECE seçilen taş türünü harcar (Void **ve** Light aynı anda DEĞİL)
  Future<void> _forge(EqItem it, String kind) async {
    ...
    if (!ok) {
      ...
      setState(() {}); // sayaçları yenile
      return;
    }

    setState(() {
      it.setKind = kind;
      // Eğer takılı olan bu item’ın aynısı ise takılı metni de güncelle
        ...
        child: ListView(
          ...
          children: [
            ListTile(
              ...
              subtitle: const Text(
                r'$100 • 10K Crystal • Mythic Helmet • Stone Box (100)',
              ),
              onTap: () async {
            ...
            ListTile(
              ...
              subtitle: const Text(
                r'$50 • 5K Crystal • 50 Helmet Shards • Stone Box (50)',
              ),
              onTap: () async {
                Navigator.pop(context);
  ...
  Widget _empty(ColorScheme cs) => Container(
    ...
    child: const Row(
      children: [
        ...
        SizedBox(width: 12),
        Expanded(child: Text('No helmet equipped.')),
      ],
    ),
  );
  ...
  Widget _rarityIcon(EqRarity r) => CircleAvatar(
    ...
    child: Icon(Icons.military_tech, color: r.color(context)),
  );
  ...
  Widget _filtersRow() {
    Widget chip(EqRarity r) => FilterChip(
      ...
      avatar: Icon(Icons.military_tech, size: 18, color: r.color(context)),
    );
    return Wrap(
      spacing: 10,
      runSpacing: 10,
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                Icon(Icons.military_tech, color: it.rarity.color(context)),
            ...
            child: Column(
              ...
              children: [
                ListTile(
                  ...
                  title: Text('Helmet · ${it.rarity.label}'),
                ...
                ListTile(
                  ...
                  subtitle: const Text('Costs: 100× VoidStone • +1% True DMG.'),
                ...
                ListTile(
                  ...
                  subtitle: const Text(
                    'Costs: 100× LightStone • +2% Holy DMG.',
                  ),
                  onTap: it.rarity == EqRarity.mythic
                      ? () => _forge(it, 'light')
                      : null,
  ...
  List<String> _bonusLines(EqItem it) {
    ...
    final acc = (0.01 * t) + (0.002 * it.level);
    final res = (0.01 * t) + (0.002 * it.level);
    final speed = 4 + it.level;
    final lines = <String>[
      'Speed: +$speed',
      'Accuracy: +${pct(acc)}',
      'Effect Res: +${pct(res)}',
    ];
    if (it.setKind == 'void') lines.add('Set: Void — +1% True DMG.');
    if (it.setKind == 'light') lines.add('Set: Light — +2% Holy DMG.');
    return lines;
  }
}
