// lib/features/equipment/armor_page.dart
import 'dart:convert';
...
class ArmorPage extends StatefulWidget {
  const ArmorPage({super.key, required this.heroName});
...
class _ArmorPageState extends State<ArmorPage> {
  static const _eqKeyPrefix = 'eq_armor_'; // slot key per hero
  static const _invKey = 'inv_armor'; // global inventory list

  // Seed örnekleri (sadece ilk girişte; sonrası persist’ten gelir)
  List<EqItem> _items = [
  ...
  EqItem? _equipped; // takılı olan (envanterde tutulmaz)
  ...
  // ----------- equality & inventory helpers -----------
  bool _same(EqItem a, EqItem b) =>
      a.rarity == b.rarity && a.level == b.level && a.setKind == b.setKind;

  /// Envanterde eşleşen BİR kopyayı bulup çıkarır; bulursa true döner
  ...
  Future<void> _loadAll() async {
    ...
    if (rawEq != null && rawEq.trim().isNotEmpty) {
      try {
        ...
        // Eski kayıtlarda aynı item envanterde kalmış olabilir → bir kopyayı düş
  ...
  // ------------------------------ actions ------------------------------
  /// Equip: envanterden 1 kopya düş, eski takılıyı envantere iade et
  Future<void> _equip(EqItem it) async {
    _removeOneFromInventory(it); // yoksa da devam: dupe oluşmaz
    ...
    ScaffoldMessenger.of(
      context,
    ).showSnackBar(const SnackBar(content: Text('Armor equipped.')));
    setState(() {});
  }

  /// Unequip: takılı item’ı envantere geri koy
  ...
  /// Forge: SADECE seçilen taş türünü harcar (Void/Light tek tür)
        ...
        child: ListView(
          ...
          children: [
            ListTile(
              ...
              subtitle: const Text(
                r'$100 • 10K Crystal • Mythic Armor • Stone Box (100)',
              ),
              onTap: () async {
            ...
            ListTile(
              ...
              subtitle: const Text(
                r'$50 • 5K Crystal • 50 Armor Shards • Stone Box (50)',
              ),
              onTap: () async {
                Navigator.pop(context);
  ...
  Widget _empty(ColorScheme cs) => Container(
    ...
    child: const Row(
      children: [
        Icon(Icons.shield),
        SizedBox(width: 12),
        Expanded(child: Text('No armor equipped.')),
      ],
    ),
  );
  ...
  Widget _rarityIcon(EqRarity r) => CircleAvatar(
    ...
    child: Icon(Icons.shield, color: r.color(context)),
  );
  ...
  Widget _filtersRow() {
    Widget chip(EqRarity r) => FilterChip(
      ...
      avatar: Icon(Icons.shield, size: 18, color: r.color(context)),
    );
    return Wrap(
      spacing: 10,
      runSpacing: 10,
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                Icon(Icons.shield, color: it.rarity.color(context)),
  ...
  Future<void> _sheet(EqItem it) async {
    ...
    await showModalBottomSheet(
      context: context,
      isScrollControlled: true, // overflow çözümü
            ...
            child: Column(
              ...
              children: [
                ListTile(
                  ...
                  title: Text('Armor · ${it.rarity.label}'),
                ...
                ListTile(
                  ...
                  subtitle: const Text(
                    'Costs: 100× VoidStone • +2% DMG Immunity.',
                  ),
                ...
                ListTile(
                  ...
                  subtitle: const Text(
                    'Costs: 100× LightStone • +3% Holy DMG.',
                  ),
                  onTap: it.rarity == EqRarity.mythic
                      ? () => _forge(it, 'light')
                      : null,
  ...
  List<String> _bonusLines(EqItem it) {
    ...
    final block = 0.02 * (1 + it.level / 10) * t;
    final lines = <String>[
      'HP: scales with hero HP (≈ (6% + 2%×Lv) × rarity factor)',
      'DEF: scales with hero DEF (≈ (6% + 2%×Lv) × rarity factor)',
      'Block: +${pct(block)}',
    ];
    if (it.setKind == 'void') lines.add('Set: Void — +2% DMG Immunity.');
    if (it.setKind == 'light') lines.add('Set: Light — +3% Holy DMG.');
    return lines;
  }
}

// Üst sayaç pill (Future tabanlı)
